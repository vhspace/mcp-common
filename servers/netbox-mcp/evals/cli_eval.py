...
The agent gets a one-shot ``bash`` tool and the netbox-cli skill as a system
prompt. It must use ``netbox-cli`` shell commands to answer infrastructure
questions.
"""
...
from _netbox_env import apply_resolved_token_to_environ, prepend_repo_bin_to_path
...
# the agent calls 'bash', not MCP tools (togethercomputer/mcp-common#59).
#
...
# command must NOT earn tool-selection credit (togethercomputer/mcp-common#133 flipped
...
def netbox_cli_eval() -> Task:
    ...
    # ...and make the model's bare `netbox-cli` resolve to the REPO's current
    # build by putting the repo venv bin first on $PATH. Without this the bash
    # sandbox can pick up a stale global netbox-cli (netbox-mcp#137: an old
    # /usr/local/bin build predating the #125 lookup->lookup-device rename),
    # silently testing the wrong code. The matrix runner also asserts this via
    # the version preflight; doing it here covers standalone `inspect eval` runs.
    ...
    return Task(
        ...
        solver=[
            ...
            # One-shot ``bash`` (command -> stdout), NOT the interactive
            # ``bash_session`` (type/type_submit/read actions). The interactive
            # session broke weak fast-tier agents two ways (netbox-mcp#138):
            # (1) they hallucinated a tool literally named ``netbox-cli``
            # instead of calling ``bash_session`` (inspect rejects it: "Tool
            # netbox-cli not found"), and (2) when they did call it they ran a
            # single ``type_submit`` and never issued the follow-up ``read``, so
            # they never saw the command output. One-shot ``bash`` returns
            # stdout directly, removing both failure modes — it lifted
            # Llama-3.3-70B cli from 0.156 to 0.906 (mcp parity) with no change
            # for the already-strong models. The CLI-aware scorer credits either
            # shell tool (mcp-common ``_BASH_TOOL_NAMES``).
