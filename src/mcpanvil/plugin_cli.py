...
from mcpanvil.marketplace_builder import build_all as build_marketplace_all
from mcpanvil.plugin_gen import (
...
def init(
    ...
    hook_block = """  - repo: https://github.com/vhspace/mcpanvil
    rev: v0.7.0
    hooks:
      - id: mcp-plugin-gen
"""
...
def audit(
    ...
    """Check mcpanvil feature adoption in this MCP server.

    Scans src/ for mcpanvil imports and reports missing features
    that every MCP server should be using.
    """
    from mcpanvil.plugin_audit import audit_repo

    repo_root = repo_root.resolve()
    result = audit_repo(repo_root)

    typer.echo("mcpanvil feature audit")
    typer.echo("=" * 40)
