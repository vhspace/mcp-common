...
a read-only eval that still exposes a **write** verb and eval-irrelevant noise
tools pushes small/fast models toward mis-selection. A read-only eval, for
instance, might otherwise expose its full tool surface — including a
write-tagged ``update`` verb and changelog readers — to the model under test.
...
line** instead of hand-maintaining tool lists::
    ...
    from mcpanvil.testing.eval import read_only_tools
...
Mirrors ``mcpanvil.dual_mode._enforce.WRITE_TAG`` — duplicated here (with a
