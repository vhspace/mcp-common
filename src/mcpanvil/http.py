...
def user_agent(component: str | None = None) -> str:
    ...
    Every MCP HTTP client MUST send an explicit ``User-Agent``: many WAFs (e.g.
    Cloudflare) ban the default ``Python-urllib/*`` UA and return ``403`` (CF
    Error 1010, ``browser_signature_banned``). This helper returns a non-default
    UA derived from the real installed ``mcpanvil`` version so the token never
    goes stale.
    ...
    Returns:
        ``"mcpanvil/<version>"`` (e.g. ``"mcpanvil/0.25.0"``), or
        ``"<component> mcpanvil/<version>"`` when *component* is given.
    """
    base = f"mcpanvil/{get_version('mcpanvil')}"
    if component:
...
def create_http_app(
    ...
    *,
    ...
    Args:
        ...
        settings: Optional :class:`~mcpanvil.config.MCPSettings`. When set and
    ...
    if enable_access:
        log = access_logger or logging.getLogger("mcpanvil.http.access")
        app.add_middleware(
...
def _with_default_user_agent(
    ...
    """Return *headers* ensuring an explicit ``User-Agent`` is set.

    The default ``python-httpx/*`` / ``Python-urllib/*`` User-Agent is banned by
    some WAFs (e.g. Cloudflare, 403, CF Error 1010). Every outbound client
    therefore sends the standardized :func:`user_agent` token by default. An
    explicit caller-supplied ``User-Agent`` header (any casing) is always
    preserved.
    """
...
class RetryingHttpxClient:
    ...
    so requests survive WAF UA bans; pass *component* to
    label it (``"<component> mcpanvil/<version>"``), or supply an explicit
