"""Shared MCP↔CLI parity test helpers for dual-mode tools (mcp-common#106).

Every MCP adopting the dual-mode framework (``@dual_mode_tool`` +
:func:`mcp_common.dual_mode.build_cli_from_mcp`) needs the same assertion: the
...
Install with ``uv add "mcp-common[testing]"``; the helpers import :mod:`fastmcp`
and :mod:`typer`, both already core dependencies of mcp-common.
"""
