...
Validates that the features ufm-mcp relies on work as advertised:
...
def test_json_formatter_emits_required_fields() -> None:
    ...
    test_logger = logging.getLogger("ufm.smoke.json")
    ...
    assert record["logger"] == "ufm.smoke.json"
...
def test_timed_operation_emits_timing_fields() -> None:
    ...
    test_logger = logging.getLogger("ufm.smoke.timing")
    test_logger.handlers.clear()
    test_logger.addHandler(handler)
    test_logger.setLevel(logging.INFO)
    test_logger.propagate = False
...
def test_timed_operation_marks_failure_on_exception() -> None:
    ...
    test_logger = logging.getLogger("ufm.smoke.timing.fail")
    test_logger.handlers.clear()
    test_logger.addHandler(handler)
    test_logger.setLevel(logging.INFO)
    test_logger.propagate = False
