"""Account for LLM-as-judge token usage in eval runs.

``summary.json`` (written by the eval runner) records token/cost only for the
**model under test** — it reflects Inspect's ``model_usage``. The LLM judge runs
through a **separate** OpenAI-compatible client in
:mod:`mcpanvil.testing.eval.scorers` (``_get_llm_client`` / ``_call_llm_judge``,
used by ``tool_use_scorer`` / ``combined_scorer`` / ``parity_scorer``) and its
token usage was never counted, so ``summary.json`` undercounts true end-to-end
cost. That matters when the judge is a **paid** model, not a free/local one, and
it feeds the cost/trend charts.
...
Usage in a runner::
    ...
    from mcpanvil.testing.eval import install_judge_usage_tracking, reset_judge_usage
