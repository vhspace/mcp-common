...
from tests.conftest import _backdate_mtime
...
class TestIrenVendorHandler:
    ...
    def test_expired_cookies_not_loaded(self, mock_credentials, tmp_path):
        ...
        _backdate_mtime(handler.cookie_file, timedelta(hours=9))
...
class TestFetchArticleDirect:
    ...
    def test_rest_api_failure_falls_back_to_browser(self, mock_credentials, tmp_path):
        ...
        with (
            ...
            patch.object(handler, "_fetch_article_via_browser", return_value=None) as mock_browser,
        ):
    ...
    def test_rest_api_none_response_falls_back(self, mock_credentials, tmp_path):
        ...
        with (
            patch.object(handler, "_freshdesk_request", return_value=None),
            patch.object(handler, "_fetch_article_via_browser", return_value=None) as mock_browser,
        ):
...
class TestGetKbArticleDirectFetch:
    ...
    def test_accepts_full_url(self, mock_credentials, tmp_path):
        ...
        with patch.object(
            handler, "_fetch_article_direct", return_value=direct_article
        ) as mock_direct:
...
class TestIrenListTicketsApi:
    ...
    def test_builds_correct_url_with_per_page(self, mock_credentials, tmp_path):
        ...
        # fetch-one-extra (#93): list_tickets over-fetches by one to detect
        # truncation, so a --limit 25 request asks the API for 26.
        assert "per_page=26" in path_arg
    ...
    def test_limit_caps_results(self, mock_credentials, tmp_path):
        handler = _make_iren_handler(mock_credentials, tmp_path, api_key="test-key")
        api_tickets = [{"id": i, "subject": f"T{i}", "status": 2} for i in range(1, 50)]
    ...
    def test_api_failure_falls_back_to_browser(self, mock_credentials, tmp_path):
        ...
        # fetch-one-extra (#93): the browser fallback is asked for limit + 1.
        mock_browser.assert_called_once_with(status=None, limit=11)
        assert result == [{"id": "99"}]
    ...
    def test_api_none_response_falls_back_to_browser(self, mock_credentials, tmp_path):
        ...
        with (
            patch.object(handler, "_freshdesk_request", return_value=None),
            patch.object(handler, "_list_tickets_via_browser", return_value=[]) as mock_browser,
        ):
    ...
    def test_no_api_key_skips_api_uses_browser(self, mock_credentials, tmp_path):
        ...
        with (
            patch.object(handler, "_list_tickets_via_api") as mock_api,
            patch.object(handler, "_list_tickets_via_browser", return_value=[]) as mock_browser,
        ):
...
@pytest.mark.unit
class TestIrenListMoreSignal:
    """IREN keeps paginating (issue #59) and now reports has_more/total (#93)."""

    def test_has_more_true_when_capped(self, mock_credentials, tmp_path):
        handler = _make_iren_handler(mock_credentials, tmp_path, api_key="test-key")
        api_tickets = [{"id": i, "subject": f"T{i}", "status": 2} for i in range(1, 50)]  # 49
        ...
        assert len(result) == 5
        assert handler.last_list_has_more is True
        # Freshdesk's list endpoint exposes no body-level total.
        assert handler.last_list_total is None

    def test_has_more_false_when_exhausted(self, mock_credentials, tmp_path):
        handler = _make_iren_handler(mock_credentials, tmp_path, api_key="test-key")
        api_tickets = [{"id": i, "subject": f"T{i}", "status": 2} for i in range(1, 4)]  # 3

        with patch.object(
            handler, "_freshdesk_request", return_value=_mock_response(200, api_tickets)
        ):
            result = handler.list_tickets(limit=20)

        assert len(result) == 3
        assert handler.last_list_has_more is False
        assert handler.last_list_total is None

    def test_still_paginates_across_pages_and_flags_has_more(self, mock_credentials, tmp_path):
        handler = _make_iren_handler(mock_credentials, tmp_path, api_key="test-key")
        page1 = [{"id": i, "subject": f"T{i}", "status": 2} for i in range(1, 101)]  # 100
        page2 = [{"id": i, "subject": f"T{i}", "status": 2} for i in range(101, 121)]  # 20
        ...
        with patch.object(handler, "_freshdesk_request", side_effect=side_effect):
            result = handler.list_tickets(limit=110)

        # 110 > a single 100-row Freshdesk page → pagination still works.
...
class TestIrenGetTicketApi:
    ...
    def test_api_none_response_falls_back_to_browser(self, mock_credentials, tmp_path):
        ...
        with (
            patch.object(handler, "_freshdesk_request", return_value=None),
            patch.object(handler, "_get_ticket_via_browser", return_value=None) as mock_browser,
        ):
    ...
    def test_include_requester_in_api_url(self, mock_credentials, tmp_path):
        ...
        ticket_call = next(c for c in mock_req.call_args_list if "/conversations" not in c[0][1])
...
class TestParseTicketFromPage:
    ...
    def _setup_handler_with_page(self, mock_credentials, tmp_path, html: str):
        ...
        def mock_evaluate(js_code):
            ...
            badge_match = re_mod.search(r'class="fw-status-badge"[^>]*>(.*?)</', html)
            if badge_match:
            ...
            for classes, content in blocks:
                ...
                # Author
                author_match = re_mod.search(r'class="(?:name|author-name)"[^>]*>(.*?)</', content)
                if author_match:
                ...
                if time_match:
                    ...
                    ts_match = re_mod.search(r'class="timestamp"[^>]*>(.*?)</', content)
                    if ts_match:
