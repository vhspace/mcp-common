...
class TestEnumerateAllLogServices:
    ...
    def test_includes_system_log_services(self, mock_host):
        ...
        # System services enumerated first
        assert result[0] == ("EventLog", "/redfish/v1/Systems/1/LogServices/EventLog")
        assert result[1] == ("Log1", "/redfish/v1/Managers/1/LogServices/Log1")
...
class TestDiscoverLogServiceSupermicro:
    def _mock_supermicro_base(self, mock_host, *, system_log_services=None):
        ...
        responses.add(
            responses.GET,
            f"{base}/redfish/v1/Systems/1/LogServices",
            json={"Members": system_log_services or []},
        )
    ...
    @responses.activate
    def test_prefers_system_sel_over_manager_mel(self, mock_host):
        """When both System and Manager expose Log1, prefer System (IPMI SEL)."""
        base = f"https://{mock_host}"
        self._mock_supermicro_base(
            mock_host,
            system_log_services=[
        ...
        url, svc = _discover_log_service(c, None)
        assert svc == "Log1"
        assert url == f"{base}/redfish/v1/Systems/1/LogServices/Log1/Entries"
...
class TestMcpGetBmcLogsSupermicro:
    ...
    @responses.activate
    @pytest.mark.anyio
    async def test_prefers_system_sel_over_manager_mel(self, mcp_tools, mock_host):
        """MCP tool should read System SEL, not Manager MEL, when both have Log1."""
        base = f"https://{mock_host}"
        responses.add(
            responses.GET,
            f"{base}/redfish/v1/Managers/iDRAC.Embedded.1/LogServices/Sel",
        ...
        responses.add(
            responses.GET,
            f"{base}/redfish/v1/Systems/1/LogServices/Log1/Entries",
            json={
                "Members": [
                    {
                        "Id": "42",
                        ...
                        "Severity": "Critical",
                        "Message": "PCIe Correctable Error (GPU slot)",
        ...
        assert result["entries"][0]["message"] == "PCIe Correctable Error (GPU slot)"
        assert result["entries"][0]["sensor_type"] == "Critical Interrupt"
