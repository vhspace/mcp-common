...
prefer-cli-over-mcp skill. ``cli_tool_use_scorer`` (mcp-common >= v0.27.0)
credits tool selection whether the agent answered via a ``netbox-cli``
subcommand or a direct MCP tool call (``accept_mcp_names=True``), so the CLI
half is no longer scored ~0 the way the MCP-name match did
(vhspace/mcp-common#59).
"""
...
def netbox_combined_eval() -> Task:
    ...
    return Task(
        ...
        scorer=cli_tool_use_scorer(cli_binary="netbox-cli", accept_mcp_names=True),
        message_limit=20,
    )
