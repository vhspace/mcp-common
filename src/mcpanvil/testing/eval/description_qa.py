...
def check_description_quality_llm(
    ...
    *,
    ...
    OpenAI-compatible judge model for multi-dimensional quality evaluation.

    Args:
        server_module: Dotted Python import path to the MCP server module.
        model: Override the judge model (default: ``EVAL_JUDGE_MODEL`` env var
            or ``Qwen/Qwen3-235B-A22B-Instruct-2507``).
        api_key: Override the API key (default: ``EVAL_JUDGE_API_KEY`` env var).
        base_url: Override the API base URL (default: ``EVAL_JUDGE_BASE_URL``
            env var; required — there is no default endpoint).
    ...
    resolved_key = api_key or os.environ.get("EVAL_JUDGE_API_KEY", "")
    if not resolved_key:
        _log.warning(
            "EVAL_JUDGE_API_KEY not set — skipping LLM-based description scoring for %s",
            server_module,
        )
        return []
