...
class TestCreateServiceRequestIren:
    ...
    def test_cli_iren_success_json(self, mock_get_handler):
        ...
        result = runner.invoke(
            ...
            [
                "create-service-request",
                "--summary",
                "GPU down on dfw01-cpu-04",
                "--description",
                "Node won't boot after power cycle",
                "--vendor",
                "iren",
                "--json",
            ],
    ...
    def test_cli_iren_failure_json(self, mock_get_handler):
        ...
        mock_handler = MagicMock(spec=IrenVendorHandler)
        mock_handler.create_ticket.return_value = None
        # Use a non-auth ``last_error`` so this test still verifies the
        # generic JSON error-surfacing path (auth-flavored errors take
        # the new exit-2 path tested in test_cli_auth_surface.py, #87).
        mock_handler.last_error = "Freshdesk API error: 500 Internal"
        mock_get_handler.return_value = mock_handler
        ...
        assert "Freshdesk API error" in data["detail"]
