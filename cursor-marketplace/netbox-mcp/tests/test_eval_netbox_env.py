...
def test_netbox_mcp_env_forwards_plain_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("NETBOX_TOKEN", OPREF)
    # No enforce toggle set in the parent -> the key must NOT appear in the child
    # env (we never inject a default; an unset toggle is byte-identical to today).
    monkeypatch.delenv(ENFORCE_READONLY_ENV, raising=False)
    monkeypatch.setattr(
        "mcp_common.credential_chain._read_op_reference",
    ...
    assert ENFORCE_READONLY_ENV not in child_env


@pytest.mark.parametrize("value", ["1", "strict", "true", "enabled"])
def test_netbox_mcp_env_forwards_enforce_readonly_when_set(
    monkeypatch: pytest.MonkeyPatch, value: str
) -> None:
    """MCP_ENFORCE_READONLY is forwarded VERBATIM to the spawned child when set.

    Without this, the MCP stdio child only inherits get_default_environment()'s
    safelist (HOME/PATH/...), so the toggle reads as unset (EnforceMode.OFF) and
    the server-side ReadOnlyEnforcementMiddleware (mcp-common #148) is inert in
    MCP-mode evals (netbox-mcp#146). Forwarding it verbatim lets the child honor
    1/strict/etc. (the value drives current_enforce_mode in the child).
    """
    monkeypatch.setenv("NETBOX_URL", "https://netbox.test")
    monkeypatch.setenv("NETBOX_TOKEN", "plain-token")
    monkeypatch.setenv(ENFORCE_READONLY_ENV, value)
    child_env = netbox_mcp_env()
    assert child_env[ENFORCE_READONLY_ENV] == value


def test_netbox_mcp_env_omits_enforce_readonly_when_unset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """An unset toggle is NOT injected (no default) — unchanged behavior (#146)."""
    monkeypatch.setenv("NETBOX_URL", "https://netbox.test")
    monkeypatch.setenv("NETBOX_TOKEN", "plain-token")
    monkeypatch.delenv(ENFORCE_READONLY_ENV, raising=False)
    assert ENFORCE_READONLY_ENV not in netbox_mcp_env()


def test_netbox_mcp_env_empty_enforce_readonly_not_forwarded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """An empty-string toggle (the SDK rejects ``""`` values) is not forwarded.

    mcp-common's current_enforce_mode() also treats "" as OFF, so omitting it
    keeps the child env minimal and matches the toggle-unset semantics.
    """
    monkeypatch.setenv("NETBOX_URL", "https://netbox.test")
    monkeypatch.setenv("NETBOX_TOKEN", "plain-token")
    monkeypatch.setenv(ENFORCE_READONLY_ENV, "")
    assert ENFORCE_READONLY_ENV not in netbox_mcp_env()


def test_enforce_readonly_env_matches_mcp_common_canonical() -> None:
    """Guard against drift from mcp-common's canonical ENFORCE_READONLY_ENV_VAR (#148)."""
    from mcp_common.dual_mode import ENFORCE_READONLY_ENV_VAR

    assert ENFORCE_READONLY_ENV == ENFORCE_READONLY_ENV_VAR
