...
:func:`mcpanvil.testing.eval.model_configs.generate_config_for_tier` emits a
**vLLM-tuned** preset: it disables hybrid-model "thinking" via
``extra_body={"chat_template_kwargs": {"enable_thinking": False}}`` — the vLLM
chat-template switch — and omits ``reasoning_effort`` because some serverless
inference APIs 400 on ``reasoning_effort="none"`` for certain models.

Eval matrices may add **Claude Haiku (fast)** and **Claude Sonnet (medium)** as
models *under test*. For an Anthropic-routed (or OpenAI-routed) model the
vLLM-only ``extra_body`` is meaningless and may error — the provider never sees
a vLLM chat template. This helper makes the preset **provider-aware** without
duplicating the per-tier reliability levers: it builds on
:func:`generate_config_for_tier` and strips the vLLM-only ``extra_body`` for
providers that don't use it, leaving every other lever (``temperature=0``, the
tier ``max_tokens`` cap, opt-in ``reasoning_effort``) intact.

This mirrors the judge ``response_format`` provider-aware behavior: keep the
vLLM default, special-case the providers whose OpenAI-compatible surface
differs.

Usage::

    from mcpanvil.testing.eval import generate_config_for_provider_tier

    vllm_cfg = generate_config_for_provider_tier("fast", "vllm")           # vLLM extra_body kept
...
if TYPE_CHECKING:
    ...
    from mcpanvil.testing.eval.model_configs import ReasoningEffort, Tier
...
VLLM_CHAT_TEMPLATE_PROVIDERS: frozenset[str] = frozenset({"vllm", "vllm-openai"})
"""Providers whose backend honors the vLLM ``chat_template_kwargs`` switch.

Only these keep the vLLM ``extra_body={"chat_template_kwargs":
...
provider is treated as vLLM-style so behaviour for existing vLLM runners is
...
# fails safe to the historical vLLM behaviour instead of silently dropping
# the thinking-off lever for a vLLM model.
...
def provider_uses_vllm_chat_template(provider: str) -> bool:
    ...
    ``True`` for vLLM-style providers (and unknown providers, which default to
    the historical vLLM behaviour); ``False`` for the known non-vLLM providers
    in :data:`_NON_VLLM_PROVIDERS` (Anthropic, OpenAI, …), whose endpoints don't
    use the switch and may reject the unknown body field. Case- and
    ...
    if normalized in VLLM_CHAT_TEMPLATE_PROVIDERS:
        return True
    # Unknown provider: preserve the vLLM-safe default.
    return True
...
def generate_config_for_provider_tier(
    ...
    * **vLLM** (and unknown providers): keep
    ...
    * **Anthropic / OpenAI / other non-vLLM providers**: drop ``extra_body``
    ...
    Args:
        ...
        provider: Inspect model provider the model-under-test routes through
            (e.g. ``"vllm"``, ``"anthropic"``, ``"openai"``). Case- and
            whitespace-insensitive; unknown providers preserve the vLLM
            default.
