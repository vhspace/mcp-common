...
def _pick_fields(
    data: dict[str, Any] | list[Any], fields: list[str] | None
...
def _apply_fields_filter(
    resp: dict[str, Any] | list[Any], fields: str | None
) -> dict[str, Any] | list[Any]:
...
def _format_template_line(t: dict[str, Any]) -> str:
    tid = t.get("id", "?")
    name = t.get("name", "?")
...
def _format_resource_line(r: dict[str, Any]) -> str:
    rid = r.get("id", "?")
...
def _format_completion_summary(job_data: dict[str, Any]) -> str:
...
def _warn_zero_hosts(job_id: int, job_data: dict[str, Any]) -> None:
...
def _warn_empty_inventory(client: AwxRestClient, ttype: str, template_id: int) -> None:
...
def launch(
    ...
    typer.echo(
        f"Timed out after {timeout}s (job_id={job_id}). Last status: {last_status}", err=True
    )
    if json_output:
        _output(
            {
...
def hosts(
    ...
    filtered = _apply_fields_filter(resp, fields)
    _output(filtered, as_json=json_output)
