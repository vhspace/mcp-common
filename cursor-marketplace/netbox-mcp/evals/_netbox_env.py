...
# Enforced-read-only toggle (mcp-common #148). Mirrors the canonical
# ``mcp_common.dual_mode.ENFORCE_READONLY_ENV_VAR`` — kept as a local literal so
# this env-builder stays importable without pulling in the dual_mode/fastmcp
# import chain; ``tests/test_eval_netbox_env.py`` asserts the two never drift.
...
def netbox_mcp_env() -> dict[str, str]:
    ...
    Also forwards the enforced-read-only toggle (:data:`ENFORCE_READONLY_ENV`)
    **when set**, so the spawned child's ``ReadOnlyEnforcementMiddleware``
    (mcp-common #148) actually activates — see :func:`_forward_enforce_readonly`.
    """
    token = resolve_netbox_token(required=False)
    if token is None:
    ...
    _forward_enforce_readonly(env)
    return env


def _forward_enforce_readonly(env: dict[str, str]) -> dict[str, str]:
    """Forward :data:`ENFORCE_READONLY_ENV` (when set) into the child *env* dict.

    The MCP Python SDK launches the stdio child with
    ``{**get_default_environment(), **explicit_env}``, and
    ``get_default_environment()`` is only a safelist (``HOME``/``PATH``/… —
    *not* ``MCP_ENFORCE_READONLY``). So unless we forward the toggle explicitly,
    the spawned ``netbox-mcp`` child reads it as unset → ``EnforceMode.OFF`` and
    its ``ReadOnlyEnforcementMiddleware`` (mcp-common #148) is a transparent
    pass-through — leaving the server-side read-only backstop **inert** during
    MCP-mode evals (netbox-mcp#146). Forwarding it activates the middleware so a
    ``{"write"}``-tagged tool (e.g. ``netbox_update_device``) is refused
    server-side even if exposed, making "trim (#131) + enforce (#148)" genuinely
    belt-and-suspenders in the harness.

    This is the same env-forwarding class as the NetBox creds (#117 / #108): the
    stdio child only inherits a safelist, so anything it needs must be passed
    explicitly. The value is forwarded **verbatim** (so ``1`` / ``strict`` /
    etc. all reach the child's :func:`current_enforce_mode`), and **only when
    set** — we never inject a default, so an unset toggle stays byte-identical
    to today's behavior.
    """
    enforce = os.environ.get(ENFORCE_READONLY_ENV)
    if enforce:
        env[ENFORCE_READONLY_ENV] = enforce
    return env
