...
Why serial: the LLM-as-judge (Together) rate-limits under concurrency, so every
eval runs with ``--max-connections 1``. The judge is fixed to ``JUDGE_MODEL``
(via the ``EVAL_JUDGE_MODEL`` env var that ``mcp_common``'s scorers read) so
scores are comparable across the models under test.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="inspect max-connections (default 1 — the judge rate-limits, keep serial).",
    )
    parser.add_argument(
        "--judge-model",
        default=JUDGE_MODEL,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"max-connections: {args.max_connections} (serial)")
    ...
    for model in runnable:
        for mode in modes:
            run_no += 1
            print(f"\n[{run_no}/{n_evals}] {mode:<8} {model.name}", flush=True)
            row: dict[str, Any] = {"model": model.name, "tier": model.tier, "mode": mode}
            try:
