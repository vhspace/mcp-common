...
:mod:`mcpanvil.dual_mode._typer_params`; the output side routes through
:func:`mcpanvil.cli.echo_result` so ``--json`` mode is uniform across
every MCP CLI.
"""
...
from mcpanvil.cli import JsonOption, SuggestingTyperGroup, create_cli_app, echo_result
from mcpanvil.dual_mode._cli_enforce import refuse_if_read_only_blocked
from mcpanvil.dual_mode._metadata import _ToolMetadata
from mcpanvil.dual_mode._naming import to_kebab_case
...
from mcpanvil.dual_mode.cli_context import CliContext
    ...
    * Routes the return value through :func:`mcpanvil.cli.echo_result`,
    ...
    Args:
        ...
        **typer_kwargs: Extra kwargs forwarded to
            :func:`mcpanvil.cli.create_cli_app`.

    Returns:
        Typer app ready to invoke or pass to
        :func:`mcpanvil.cli.run_cli`.
    """
...
def _resolve_group(
    ...
    :func:`mcpanvil.cli.create_cli_app` puts on the top-level app) so a typo'd
...
def _enforce_cli_read_only(meta: _ToolMetadata) -> None:
    ...
    Mirrors the MCP-side :class:`mcpanvil.dual_mode._enforce.ReadOnlyEnforcementMiddleware`
    so both interfaces behave identically. Delegates to the shared
    :func:`mcpanvil.dual_mode._cli_enforce.refuse_if_read_only_blocked` (the
    same gate the public :func:`mcpanvil.dual_mode.enforce_read_only_cli`
...
def _build_command_signature(typer_params: list[inspect.Parameter]) -> inspect.Signature:
    ...
    Uses :data:`mcpanvil.cli.JsonOption` so every dual-mode CLI command
    ...
    if inspect.iscoroutine(result) or inspect.isasyncgen(result):
        ...
        if _wrapped_is_coroutine_function(fn):
            ...
            raise RuntimeError(
                ...
                f"mcpanvil.dual_mode.enforce_read_only_cli for the recipe."
            )
        raise RuntimeError(
