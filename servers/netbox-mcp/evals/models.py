...
MODELS: list[EvalModel] = [
    ...
    # --- closed / frontier-vendor — skip unless the matching key is present -
    # Broadens the matrix beyond Together open-weights with Anthropic frontier
    # coverage at the fast + medium tiers (alongside the existing high-tier
    # closed models). All are gated by ``requires_env`` so a run without the key
    # SKIPs them in ``classify_model`` instead of failing. The inspect provider
    # for Anthropic is ``anthropic/<model>``.
    #
    # NOTE — model-under-test vs judge: the default LLM-as-judge is *also* an
    # Anthropic Sonnet (``claude-sonnet-4-6``; see the #137/#140 RESULTS.md), but
    # the judge is a SEPARATE role configured via ``EVAL_JUDGE_MODEL`` /
    # ``EVAL_JUDGE_API_KEY`` (mcp_common scorers) — the entries below are the
    # models being SCORED, not the scorer.
    #
    # Slugs verified live against the Anthropic /v1/models API (2026-05-31):
    # ``claude-haiku-4-5-20251001`` ("Claude Haiku 4.5") is the only Haiku id the
    # catalog returns (no undated alias), ``claude-sonnet-4-6`` ("Claude Sonnet
    # 4.6") is the newest Sonnet, ``claude-opus-4-8`` ("Claude Opus 4.8").
    EvalModel(
        "anthropic/claude-haiku-4-5-20251001",
        "fast",
        False,
        requires_env="ANTHROPIC_API_KEY",
        note="Anthropic frontier-vendor coverage at the fast/small tier",
    ),
    EvalModel(
        ...
        requires_env="ANTHROPIC_API_KEY",
        note=(
            "Anthropic frontier-vendor coverage at the medium tier (also the "
            "default judge slug — model-under-test is scored separately from the judge)"
        ),
    ),
