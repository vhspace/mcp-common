...
# Build the redfish-cli Typer app from the FastMCP server through the mcp-common
# dual-mode framework. ``build_cli_from_mcp`` -> ``create_cli_app`` already wires
# ``no_args_is_help`` + ``SuggestingTyperGroup`` + ``install_cli_exception_handler``,
# so the previous manual Typer setup is no longer needed. The redfish MCP tools
# are registered ``mcp_only=True`` (see ``mcp_server._tool``) because redfish-cli
# is a hand-written Typer surface with different ergonomics than the MCP tools
