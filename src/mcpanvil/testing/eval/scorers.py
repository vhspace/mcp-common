...
_DEFAULT_JUDGE_MODEL = "Qwen/Qwen3-235B-A22B-Instruct-2507"
...
# the agent ran the right command. The helpers below
...
# ``mcpanvil.dual_mode._naming.derive_cli_name`` rather than importing it.
# Importing that helper transitively loads the whole dual-mode framework
# (fastmcp / typer / click) via ``mcpanvil.dual_mode.__init__``, which the
...
def _to_kebab_case(name: str) -> str:
    ...
    Mirrors ``mcpanvil.dual_mode._naming.to_kebab_case`` (see module note).
    """
...
def _strip_mcp_namespace(tool_name: str, mcp_name: str) -> str:
    ...
    Mirrors ``mcpanvil.dual_mode._naming.strip_mcp_namespace``.
    """
    if not mcp_name:
        return tool_name
...
def _derive_cli_subcommand(tool_name: str, mcp_name: str) -> str:
    ...
    ``mcpanvil.dual_mode._naming.derive_cli_name``.
    """
...
def _acceptable_subcommands(
    ...
    :func:`mcpanvil.dual_mode.tool_cli_subcommands` — so a tool whose real
...
def _compute_cli_tool_selection_score(
    ...
    MCP tool call that ran no CLI command; combined
...
def _get_llm_client() -> tuple[Any, str] | None:
    ...
    budget — which is what lets a runner raise ``max_connections`` once the judge
    is on its own budget. Each piece is resolved from the environment:

    - **API key** — ``EVAL_JUDGE_API_KEY`` (required; judge scoring is disabled
      when unset).
    - **Base URL** — ``EVAL_JUDGE_BASE_URL`` (required; the judge is
      OpenAI-compatible and there is **no** default endpoint).
    - **Model** — ``EVAL_JUDGE_MODEL`` if set, else the built-in default.

    Returns ``None`` when ``EVAL_JUDGE_API_KEY`` is not set (judge scoring
    disabled). Raises when a key is set but ``EVAL_JUDGE_BASE_URL`` is missing,
    rather than silently defaulting to a vendor endpoint.
    """
    api_key = os.environ.get("EVAL_JUDGE_API_KEY", "")
    if not api_key:
        _log.warning("EVAL_JUDGE_API_KEY not set — LLM-as-judge scoring disabled")
        return None
    ...
    if not base_url:
        raise RuntimeError(
            "EVAL_JUDGE_API_KEY is set but EVAL_JUDGE_BASE_URL is not. Set "
            "EVAL_JUDGE_BASE_URL to your OpenAI-compatible judge endpoint "
            "(e.g. https://api.openai.com/v1); there is no default endpoint."
        )
...
# Some providers return ``x-ratelimit-reset`` (the suggested retry interval) on
# a 429 and the OpenAI client attaches the raw response headers to the error
...
# delta-seconds or an HTTP-date) is the explicit directive; the provider's
...
def _parse_reset_header_value(value: str) -> float | None:
    ...
    Handles the encodings seen across the provider ecosystem:

    - **delta seconds** — ``"12"`` / ``"1.5"`` (the ``x-ratelimit-reset`` and
...
# "json_object"}``. Most OpenAI-compatible endpoints support that field, but
# Anthropic's OpenAI-compatibility endpoint (``https://api.anthropic.com/v1/``)
# does not: its compat layer documents ``response_format`` as *ignored*, and
# sending it was disruptive enough that it had to be stripped at runtime as a
# shim. Rather than push that shim onto every caller, the judge omits
# ``response_format`` automatically for endpoints known not to support it and
# keeps it everywhere else — so a native Anthropic judge
# (``EVAL_JUDGE_BASE_URL=https://api.anthropic.com/v1/``) works with no per-run
# patching. The score is parsed from the response *text* in :func:`_judge`
# (``json.loads`` of a prompt that already mandates a bare JSON object), so
# dropping the field never changes how the score is extracted.
#
...
def _judge_base_url(client: Any) -> str:
    ...
    ``EVAL_JUDGE_BASE_URL`` when it can't be read (e.g. a bare test double whose
    ``base_url`` isn't a URL).
    """
...
def _supports_json_object_response_format(base_url: str) -> bool:
    ...
    host) defaults to ``True`` to preserve the prior OpenAI-compatible behaviour.
    """
...
def _call_llm_judge(client: Any, model: str, prompt: str) -> str:
    ...
    "json_object"}`` **only when the judge endpoint supports it**. Most
    OpenAI-compatible endpoints honor that field; Anthropic's
    OpenAI-compatibility endpoint does not (it lists ``response_format`` as
    ignored and it has to be stripped at runtime), so for Anthropic-style hosts
    the field is omitted automatically and a native Anthropic judge needs no
    runtime shim. Provider detection is host-based off the client's ``base_url``
    (derived from ``EVAL_JUDGE_BASE_URL``); see
    ...
    Retries transient OpenAI-compatible failures. On a 429 ``RateLimitError`` the
...
_MISSING_API_KEY_MSG = (
    ...
    "(to point the judge at a dedicated key/endpoint), or "
...
def tool_use_scorer(judge_model: str | None = None) -> Scorer:
    ...
    Evaluates:
      ...
      2. **Task completion** — the final output satisfies the user's request
         (assessed by an LLM judge via an OpenAI-compatible endpoint).
...
def cli_tool_use_scorer(
    ...
    command was run, dragging accuracy down. This scorer
    ...
    Args:
        ...
        accept_mcp_names: Also credit an expected tool when its MCP name was
            ...
            actually exist. It defaults to ``False``: in a CLI-only eval the MCP tools are not
        ...
        tool_subcommands: Optional ``{mcp_tool_name: [cli_subcommand, ...]}``
            ...
            :func:`mcpanvil.dual_mode.tool_cli_subcommands` from the
...
# imports are deferred into :mod:`mcpanvil.testing.eval.deepeval_backend`,
...
# OpenAI-compatible (``EVAL_JUDGE_*``-aware) client the other scorers use.


def _deepeval_to_score(result: Any, answer: str) -> Score:
    """Map a :class:`~mcpanvil.testing.eval.deepeval_backend.DeepEvalResult` to an Inspect ``Score``.
...
def faithfulness_scorer(judge_model: str | None = None, threshold: float = 0.5) -> Scorer:
    ...
    Requires the ``eval-scoring`` extra; the judge is the OpenAI-compatible LLM
    ...
    async def score(state: TaskState, target: Target) -> Score:
        client, model_name = _require_llm_client(judge_model)
        from mcpanvil.testing.eval import deepeval_backend as _de
