...
(mcp-common #85) where the redfish skill had a name/path mismatch
(``name = "redfish-operations"`` pointing at ``skills/redfish-bmc-ops/SKILL.md``)
and the ``redfish-operations`` dir was not registered at all. The convention
(netbox/awx) is: one entry per skill dir with ``name`` == dir basename and
