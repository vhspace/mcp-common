...
the way the MCP-name match did (togethercomputer/mcp-common#59).
...
* ``accept_mcp_names=True`` — set explicitly because v0.28.0 flipped the
  default to ``False`` (togethercomputer/mcp-common#133). A combined run SHOULD still
...
def netbox_combined_eval() -> Task:
    ...
    apply_resolved_token_to_environ()
    # Make the bash half's bare `netbox-cli` resolve to the REPO's current build
    # (repo venv bin first on $PATH), not a stale global (netbox-mcp#137).
    prepend_repo_bin_to_path()
    return Task(
