...
GET_OBJECTS_BY_IDS_DESCRIPTION = """Fetch multiple NetBox objects by their IDs in a single call.
...
OOB_SUMMARY_DESCRIPTION = """Compact OOB-management view of a NetBox device.

Returns a structured ``DeviceOOBSummary`` (Pydantic model) with just the
fields agents need for cross-MCP workflows:

  - ``oob_ip``: BMC/IPMI/Redfish address
  - ``primary_ip4``: SSH/application address
  - ``provider_machine_id``: vendor/site-operator hostname

Use ``status_filter`` to require a specific NetBox device status; the call
fails if the resolved device's status does not match. Useful for guarding
write workflows against stale or decommissioned devices.
