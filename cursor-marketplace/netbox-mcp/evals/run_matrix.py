...
def main(argv: list[str] | None = None) -> int:
    ...
    version_preflight_state = (
        "skipped (--skip-version-preflight)"
        if args.skip_version_preflight
        else "assert repo's current netbox-cli/netbox-mcp under test (fail-fast)"
    )
    print(f"version check  : {version_preflight_state}")
    ...
    # Both preflights write their fact dicts here; the summary builder embeds
    # them so summary.json always records what was (or wasn't) validated.
    version_preflight_summary: dict[str, Any] = {"status": "not_run"}
    preflight_summary: dict[str, Any] = {"status": "not_run"}
    ...
    def build_summary(results: list[dict[str, Any]]) -> dict[str, Any]:
        return {
            ...
            "version_preflight": version_preflight_summary,
    ...
    # ---- version preflight: prove the REPO's current netbox-cli/netbox-mcp ----
    # Before spending ~75 min of live eval compute, assert the binaries under
    # test are the working-tree build, NOT a stale global PATH binary. The
    # netbox-mcp#137 cli sweep silently ran an old global netbox-cli (v2.14.1,
    # pre-#125 lookup->lookup-device rename), so its cli column never tested
    # current code. This also prepends the repo venv bin to PATH so the bash
    # sandbox's bare `netbox-cli` resolves to the repo build (fail-fast on any
    # mismatch — mirrors the NetBox credential preflight below).
    if args.skip_version_preflight:
        version_preflight_summary = {"status": "skipped"}
        print("VERSION PREFLIGHT: skipped (--skip-version-preflight)\n")
    else:
        from _netbox_env import EvalBinaryVersionError, preflight_eval_binaries

        print(_rule())
        print("VERSION PREFLIGHT: asserting repo's current netbox-cli/netbox-mcp under test ...")
        try:
            version_preflight_summary = preflight_eval_binaries()
        except EvalBinaryVersionError as exc:
            version_preflight_summary = {"status": "error", "error": str(exc)}
            print(f"VERSION PREFLIGHT FAILED: {exc}")
            print(
            ...
            return 2
        print(f"VERSION PREFLIGHT OK: {version_preflight_summary}\n")
