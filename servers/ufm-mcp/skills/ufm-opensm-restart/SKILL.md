...
This SysV-init-style script stops/starts in this order: Telemetry Sampling → UnhealthyPorts → UFM Secondary Telemetry → UFM Primary Telemetry → UFM main module → AuthenticationServer → OpenSM, then brings everything back in reverse. Total outage ~10-20s; the UFM container does not restart, so `/usr/bin/ufm_ha_watcher` stays happy and DRBD stays `Primary/Secondary UpToDate`.
