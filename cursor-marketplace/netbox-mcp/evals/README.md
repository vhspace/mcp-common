...
`--max-connections N` (default 1), `--judge-model <together/...>`,
`--log-dir <path>`, `--dry-run`, `--no-verify` (skip the live catalog check).

### Serial execution & the judge

- **Serial only.** The LLM-as-judge (Together) rate-limits (429) under concurrency, so the
  runner pins `--max-connections 1` and runs one eval per `(model, mode)` at a time.
- **Fixed judge.** Scoring is comparable across models because the judge is fixed to
  ...
  from the model under test: `mcp_common`'s scorers always call Together and read the bare
  slug from `EVAL_JUDGE_MODEL`, which the runner exports automatically.
