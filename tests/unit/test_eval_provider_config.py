...
from mcp_common.testing.eval.model_configs import _TIER_MAX_TOKENS, Tier
...
class TestProviderUsesVllmChatTemplate:
    ...
    def test_unknown_provider_defaults_to_vllm_style(self) -> None:
        # preserve the historical Together-safe behaviour for unrecognized providers
...
class TestGenerateConfigForProviderTier:
    ...
    def test_together_keeps_chat_template_extra_body(self, tier: Tier) -> None:
        # Together/vLLM honors the enable_thinking chat-template switch
        assert generate_config_for_provider_tier(tier, "together").extra_body == _VLLM_EXTRA_BODY
