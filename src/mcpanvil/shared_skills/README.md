...
Several conventions for MCP servers built on mcpanvil are easier to teach an
...
look, how to file an issue when something fails) belong to mcpanvil.

`mcp-plugin-gen` promotes every `SKILL.md` bundle
under this directory into the generated `skills/` tree of every
downstream MCP plugin, so all agents working on an mcpanvil-based MCP see the
...
- `mcpanvil-conventions` — proto-skill paired with
  [`docs/AGENT_CONVENTIONS.md`](../../../docs/AGENT_CONVENTIONS.md).
