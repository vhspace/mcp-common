...
from ufm_mcp.server import mcp, sites

# Build the CLI from the FastMCP server's dual-mode tools. Every UFM tool is
# registered as ``@dual_mode_tool(mcp_only=True)`` because each ufm-cli command
# has a bespoke name, flags, and human-readable formatting that diverge from the
# MCP tool signatures (mirroring netbox-mcp's "kept off CLI synthesis on
# purpose" pattern). ``build_cli_from_mcp`` therefore synthesizes no commands
# here, but still wires the standard ``no_args_is_help`` + ``SuggestingTyperGroup``
# (did-you-mean suggestions, structured ``--json`` errors) +
# ``install_cli_exception_handler`` (with the monorepo project_repo) onto the
# app; the hand-written ``@app.command()`` definitions below attach to it.
