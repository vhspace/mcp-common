...
The custom Inspect scorers in :mod:`mcpanvil.testing.eval.scorers`
...
own module means ``scorers.py`` — and therefore the whole ``mcpanvil.testing.eval``
...
is deferred until a DeepEval scorer is actually run. Install the backend with::
    ...
    uv pip install "mcpanvil[eval-scoring]"
...
OpenAI-compatible client that :func:`mcpanvil.testing.eval.scorers._get_llm_client`
builds (configured via the ``EVAL_JUDGE_*`` environment variables), which
...
_INSTALL_HINT = (
    ...
    'Install with: uv pip install "mcpanvil[eval-scoring]"'
)
...
def _judge_model_cls() -> type:
    ...
    *every* subclass creation, so re-defining the adapter on each
    ...
    class _OpenAICompatDeepEvalModel(DeepEvalBaseLLM):  # type: ignore[misc]  # base is untyped (optional dep)
        ...
        def generate(self, prompt: str, *args: Any, **kwargs: Any) -> str:
            ...
            from mcpanvil.testing.eval.scorers import _call_llm_judge
    ...
    return _OpenAICompatDeepEvalModel
...
def build_judge_model(client: Any, model_name: str) -> Any:
    ...
    :func:`mcpanvil.testing.eval.scorers._get_llm_client` and pointed at
    whatever ``EVAL_JUDGE_BASE_URL`` selects — so DeepEval judging rides on the
    configured judge endpoint/credentials and shares its retry/backoff +
    JSON-mode behaviour.
